...
def mart_warten(bankroll, rounds_played, wins, losses, state, min_bet, max_bet):
    """
    Modifiziertes Martingale-System: Setzt erst auf Rot, nachdem 5x hintereinander Schwarz erschien.
    """
    red_numbers = [1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36]

    # Initialisiere State-Variablen
    if not state:
        state["current_bet"] = min_bet
        state["last_win"] = False
        state["last_5_spins"] = []

    # Aktualisiere die letzten Spins (max. 5 Einträge)
    if rounds_played > 0:
        # Annahme: play_round speichert die Farbe des letzten Spins in state["last_spin_color"]
        last_color = state.get("last_spin_color", "")
        state["last_5_spins"].append(last_color)
        if len(state["last_5_spins"]) > 5:
            state["last_5_spins"].pop(0)

    # Prüfe, ob die letzten 5 Spins Schwarz waren
    activate_bet = all(color == "black" for color in state["last_5_spins"][-5:])

    # Entscheide Einsatzlogik
    if activate_bet:
        # Normales Martingale-Verhalten
        if not state["last_win"]:
            state["current_bet"] = min(state["current_bet"] * 2, max_bet, bankroll)
        else:
            state["current_bet"] = min_bet
    else:
        # Kein Einsatz oder Mindesteinsatz (abhängig von Bankroll)
        state["current_bet"] = min_bet if bankroll >= min_bet else 0

    bet_amount = state["current_bet"]
    state["last_win"] = False  # Wird in play_round aktualisiert
